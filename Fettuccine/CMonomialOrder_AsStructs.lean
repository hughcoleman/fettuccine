...
def toLinearOrder (ord : CMonomialOrder σ) : LinearOrder (CMonomial σ) :=
...
def max'₀ (supp : Finset (CMonomial σ)) (h : supp.Nonempty)
    : CMonomial σ :=
  @Finset.max' _ ord.toLinearOrder supp h

lemma max'₀_mem (supp : Finset (CMonomial σ)) (h : supp.Nonempty) : supp.max'₀ ord h ∈ supp :=
  @Finset.max'_mem _ ord.toLinearOrder supp h
...
lemma leadingMonomial_eq_some_of_nonempty (f : CMvPolynomial σ R) (hf : f.support.Nonempty) :
    leadingMonomial ord f = some (f.support.max'₀ ord hf) := by
...
lemma leadingMonomial_mem_support (f : CMvPolynomial σ R) (hf : f.support.Nonempty) :
    f.support.max'₀ ord hf ∈ f.support := by
  exact f.support.max'₀_mem ord hf
